class Emails():
    ...
    def send_to_email(self, campaign_id=None, prospect_email=None, **kwargs):
        """
        ...
        """
        kwargs.update({'prospect_email': prospect_email, 'campaign_id': campaign_id})
        result = self._post(
            path='/do/send/prospect_email/{prospect_email}'.format(prospect_email=kwargs.get('prospect_email')),
            params=kwargs)
        return result

    def send_to_id(self, campaign_id=None, prospect_id=None, **kwargs):
        """
        Sends an email to the prospect identified by <prospect_id>.
        Required parameters: (email_template_id OR (text_content, name, subject, & ((from_email & from_name) OR from_user_id)))
        """
        kwargs.update({'prospect_id': prospect_id, 'campaign_id': campaign_id})
        result = self._post(
            path='/do/send/prospect_id/{prospect_id}'.format(prospect_id=kwargs.get('prospect_id')), params=kwargs)
        return result

    def send_to_lists(self, campaign_id=None, list_ids=None, **kwargs):
        """
        Sends an email to the lists identified by <list_ids>.
        Required parameters: (email_template_id OR (text_content, name, subject, & ((from_email & from_name) OR from_user_id)))
        """
        kwargs.update({'list_ids': list_ids, 'campaign_id': campaign_id})
        result = self._post(
